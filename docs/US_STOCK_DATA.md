...
갱신 요청이 실패하거나 필드가 누락되면 기존 품질 기한(48시간) 이내의 검증된
재무 자료만 재사용한다. `fetched_at`·결산일을 변경하지 않으며 `refresh`에 실패를
남긴다. 기한 초과·미지원 통화 자료는 복구하지 않는다. 시세 요청의 연결 오류·
타임아웃은 HTTP 429/일시적 서버 오류와 같이 총 3회 안에서 재시도한다.
...
재무 자료만 부족하면 정상 시세·완료봉은 별도 `partial` job에 저장하고 부모
job은 실패로 유지한다. `data_quality.market_ready`와 `research_ready`로 시세와
재무 준비 상태를 구분한다. 전체 `ready`가 참이 아닌 자료는 투자 근거 완료로
사용하면 안 된다. 비용 미확인은 시세·재무 수집을 중단시키지 않으며 기존
`cost_verified_3r=false`로 남는다. 재무 API 복구 후 다음 주기에 자동 재수집한다.
운영 실패율에도 `partial`을 실패로 포함해 부분 수집이 성공률을 부풀리지 않게 한다.
