...
제공사가 최신 봉의 필드를 일시 누락하면 DB의 기존 완료봉을 다시 검증하여
재사용하고 `candle_recovery`에 표시한다. 봉 시각·기존 수집 시각은 갱신하지 않는다.
새 응답에 있는 수치와 충돌하는 기존 봉은 재사용하지 않는다. 기존 봉도 필수
거래일·신선도 검사를 통과해야 하며, 필수 근거가 부족한 작업은 실패로 남긴다.
